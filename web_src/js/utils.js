...
function asyncComponent(options, resolve, reject) {
    var promise;
    // var pathRegex = new RegExp(/^(\w+\/)*\w*.html$/);
...
function getPlatform() {
    ...
    var platform = addon.getPlatform();
    if(platform === 'darwin') {
        return 'OSX';
...
function dateFormat(timestamp, options) {
    ...
    if(date.isSame(now, 'week')) {
        if(date.isSame(now, 'month')) {
            var dayOfWeek = locale.time.week[date.weekday()];
    ...
    return date.format('YYYY/M/D') + time;
}
...
// 返回数组的交集
function intersection(array, another) {
    return array.filter(function (item) {
        return another.indexOf(item) >= 0;
    });
}
...
function slice(str, length) {
    ...
    var result = '';
    var size = length * 2;
    for(var list = str.split(''), i = 0; size > 0 && i < str.length; i++) {
        var item = list[i];
        result += item;
...
//目前用于群成员搜索
function searchName(arrName, keyword){
    if(!arrName.length){
        return false;
    }
    if(isEmpty(keyword)) {
    ...
    if(joinWords.indexOf(keyword) > -1){
        find = arrName.some(function(item){
            return item && item.indexOf(keyword) > -1;
        });
        return find;
    }
    var name = arrName.join(joinWords);
    if(name){
        var keywordLowerCase = keyword && keyword.toLowerCase();
        var pinyinObj = convertToABC(name);
        var pinyin = pinyinObj.pinyin.toLowerCase();
        var pinyinFirstLetter = pinyinObj.first.toLowerCase();
        var matchFirstLetter = pinyinFirstLetter.indexOf(keywordLowerCase) > -1;
        var matchPinyin = pinyin.indexOf(keywordLowerCase) === 0;
        var matchRawName = name.indexOf(keyword) > -1;
...
function replaceUri(str, callback) {
    var result = '';
...
function replaceEmail(str, callback) {
    var result = '';
...
function formatFileSize(totalSize) {
    var nameList = ['Byte', 'KB', 'MB', 'GB', 'TB'];
    var temp = 1024;
    for (var i = 0; i < nameList.length; i++) {
        var name = nameList[i];
        if (totalSize < Math.pow(temp, i + 1)) {
            return i === 0 ? totalSize + name : (totalSize / Math.pow(temp, i)).toFixed(2) + name;
        }
    }
}
...
function loadScript(url, callback) {
    ...
    // callback = callback || $.noop;
    // var deferred = $.Deferred();
    // var exist = readyScript.indexOf(url) !== -1;
    // if (exist) {
    //     callback();
    //     deferred.resolve();
    //     return deferred.promise();
    // }
    // readyScript.push(url);
...
function loadLocale(arr, callback) {
    callback = callback || $.noop;
    var def = [];
    arr.forEach(function (item) {
        var url = 'js/locale/' + item + '.js';
        def.push(loadScript(url));
    });
    $.when.apply(null, def).then(function () {
        callback();
    });
}
