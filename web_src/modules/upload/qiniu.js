(function(win) {
	...
	function uploadNextChunk(blob, opts, callback) {
		offsetStore[blob.uniqueName] = offsetStore[blob.uniqueName] || 0;
		var offset = offsetStore[blob.uniqueName];
		...
		uploadData(chunkBlob, opts, {
			onCompleted: function(chunkRes) {
				offsetStore[blob.uniqueName] += curChunkSize;
				offset += curChunkSize;
			...
			onProgress: function(chunkLoaded, total) {
				...
				// if(loaded > opts.filesize){
    //                 console.log('onProgress err', chunkLoaded, offset, opts.filesize, total);
				// }
