...
var utils = {
    ...
    secondToMinute: function (value) {
        ...
        return utils.prefixZero(Math.floor(value / 60)) + ':' + utils.prefixZero(value % 60);
    },
...
function init(config) {
    ...
    var voip = new Vue({
        ...
        created: function () {
            ...
            created(this, query.userid, query.locale);
        },
        computed: {
            locale: function () {
...
function startTiming(context) {
    ...
    setInterval(function () {
        context.time =  Math.round((new Date - startTime)/1000);
    }, 500);
}
...
var handlerCommand = {
    ...
    InviteMessage: function (command, context) {
        ...
        context.self = getUser(memberList, command.self.id);
        ring.play();
        Voip.win.setRingPos();
    },
    ...
    MemberModifyMessage: function (command, context) {
        ...
        */
        ...
        if (beInvited) {
            ...
            context.other.push.apply(context.other, inviteUserList);
            context.memberList.push.apply(context.memberList, inviteUserList);
        }
    },
...
function addMember(context) {
    ...
    Voip.request(params, function (errorCode, list) {
        ...
        context.other.push.apply(context.other, list);
...
function removeMember(context, userId) {
    ...
    if (context.senderUser.id === userId) {
        var other = context.memberList.filter(function (item) {
            return item.id !== context.self.id;
        });
...
function created(context, userId, locale) {
    watchClose(context);
    ...
    RongCallLib.videoWatch(function (result) {
        videoItem[result.type](result.data, context);
    });
